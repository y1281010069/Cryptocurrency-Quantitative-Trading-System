...
import sys
import ccxt
from strategies.condition_analyzer import calculate_ema_trend_indicators_and_score,calculate_rsi_divergence_score, calculate_volume_score
...
class MultiTimeframeStrategy(BaseStrategy):
    ...
    def _analyze_timeframe(self, df: pd.DataFrame, timeframe: str) -> tuple:
        ...
        if timeframe == self.config["SIGNAL_TRIGGER_TIMEFRAME"]:
            ...
            score += calculate_rsi_divergence_score(df) / 3
        else:
            # 非交易信号触发周期运行其他评分方法
