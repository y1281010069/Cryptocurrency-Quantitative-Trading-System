...
class MultiTimeframeStrategy(BaseStrategy):
    ...
    def analyze(self, symbol: str, data: Dict[str, pd.DataFrame]) -> Optional[MultiTimeframeSignal]:
        ...
        try:
            ...
            if all_agreed:
                target_multiplier *= 2.5  # 所有时间框架一致时，使用2.5倍的TARGET_MULTIPLIER，降低倍数以提高达成概率
            ...
            if overall_action == "买入":
                ...
                # 确保止盈止损比例合理，至少1:1
                reward_risk_ratio = target_multiplier / self.config['STOP_LOSS_MULTIPLIER']
                if reward_risk_ratio < 1.0:
                    # 调整止盈点以确保至少1:1的风险回报比
                    target_short = current_price + self.config['STOP_LOSS_MULTIPLIER'] * atr_value
            else:
                # 卖出方向：
                # - target_multiplier倍ATR作为短期目标（当前价格 - target_multiplier*ATR）
                # - STOP_LOSS_MULTIPLIER倍ATR作为止损价格（当前价格 + STOP_LOSS_MULTIPLIER*ATR）
    ...
    def _analyze_timeframe(self, df: pd.DataFrame, timeframe: str) -> tuple:
        ...
        if timeframe == self.config["SIGNAL_TRIGGER_TIMEFRAME"]:
            # 交易信号触发周期只运行RSI交叉评分
            score += calculate_rsi_crossover_score(df, window=7)
        else:
            # 非交易信号触发周期运行其他评分方法
